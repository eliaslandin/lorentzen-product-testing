...
CREATE POLICY "Allow authorized select access on test table"
ON api.tests
FOR SELECT
TO authenticated
USING ( (SELECT api.check_user_permissions('tests.select')) );

CREATE POLICY "Allow authorized insert access on test table"
ON api.tests
FOR INSERT
TO authenticated
WITH CHECK ( (SELECT api.check_user_permissions('tests.insert')) );

CREATE POLICY "Allow authorized update access on test table"
ON api.tests
FOR UPDATE
TO authenticated
USING ( (SELECT api.check_user_permissions('tests.update')) );

CREATE POLICY "Allow authorized delete access on test table"
ON api.tests
FOR DELETE
TO authenticated
